...
- **2022 — Your First Persistent Workspace (with Antigravity).** The same workshop as 2020 on a different tool — same ZIP, same prompts. Exists so a student blocked on USC IT's Codex grant can do the workshop today on a free Google account. **Steps 1–3 must stay "install the tool, download Obsidian, install Obsidian"** in both 2020 and 2022: 389's Week 2.2 page and syllabus both say "complete Steps 1–3 before class," and that has to be true whichever version a student picks. Past step 4 the numbering no longer matches 2020 and doesn't need to. **A change to 2020's workshop content needs mirroring here.**

  *Why Antigravity and not Gemini CLI (2026-09-07).* Drafted first on the CLI, on quota grounds: Gemini CLI's free tier is 1,000 requests/day against 1,500 paid, where Antigravity's free tier meters **agent invocations** — the one thing this workshop consists of — on an unpublished, repeatedly-tightened ceiling. Peter overrode that on pedagogy: no terminal is worth more to this cohort than a quota guarantee, which is the same reasoning behind the quick-start-first restructure above. The quota risk is met in troubleshooting, which offers switching models and then Tutorial 1001 (the CLI, with its far larger allowance) if a student runs dry mid-workshop. Antigravity's own affordances carry the rest better than the CLI did: the Implementation Plan artifact needs no plan mode and takes inline comments, and the Task List and Walkthrough artifacts give the watch-it-work and read-what-it-built steps something concrete to look at.

  *Keep it simple — a standing instruction for this page (2026-09-07).* It is an introduction for brand-new users, and Peter has said explicitly that a setting being merely good rather than optimal is fine. Two things were cut for exactly this reason and **should not be reinstated**: a chapter orienting students to Agent Manager vs. Editor, and three paragraphs justifying the model pick on quota-burn grounds. The model step is now one line — take the Gemini Pro one it suggests. Resist re-adding tuning advice here; the place for cost and quota nuance is 2021, whose readers have already done this once.

  *One unverified claim the page leans on:* allowances appear to pool **per model family**, so exhausting Gemini Pro may not exhaust the Claude models — that's what makes "switch models" the first troubleshooting move. Widely reported by users, undocumented by Google. The advice costs nothing to try either way; confirm in the pilot.
...
The 2000 block is not tool-scoped — those pages teach a practice, not a product, so the decade convention above doesn't apply to them. They run in sequence: **2020 or 2022** (someone else's folder) then **2021** (your own). 2020 and 2022 are the same workshop on two tools, so they are alternatives rather than steps — hence adjacent numbers rather than a shared one, which the corpus-index registry can't carry.
